...
class ChempropModel:
    ...
    def __init__(self, args: InterpretArgs) -> None:
        ...
        self.scaler, self.features_scaler, self.atom_descriptor_scaler, self.bond_descriptor_scaler, self.atom_bond_scaler = load_scalers(
            args.checkpoint_paths[0])
        self.checkpoints = [load_checkpoint(checkpoint_path, device=args.device) for checkpoint_path in
...
@timeit()
def interpret(args: InterpretArgs, visualise_smiles: bool) -> List[Dict[str, float]]:
    """
    ...
    for smiles in all_smiles:
        ...
        if len(rationales) == 0:
            ...
            if visualise_smiles:
                mol = vis.get_mol(smiles)
                submol = vis.get_mol(rats[0].smiles)
                matches = vis.find_matches_one(mol, submol)
                atomset = list(matches[0])

                # Save the visualized image to the output directory
                img = vis.get_image(mol, atomset)
                img_list.append(img)  # Store each image
