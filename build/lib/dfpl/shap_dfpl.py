...
def shap_explain(x_train, x_test, model, target, outputDir: str, drop_values=False, threshold=100, save_values=False):
    
    if drop_values:
        # x_train_to_explain = np.multiply(x_train, 0, where=np.sum(x_train, axis=0, dtype=np.int32, keepdims=True) < threshold)
        
        
        x_train_to_explain = np.where(np.sum(x_train, axis=0, dtype=np.int8, keepdims=True) >= threshold, x_train, 0)
        ...
        if save_values:
            ...
            with open(path.join(outputDir, f"shap_values-threshold-{threshold}-{target}.csv"), "wb") as csv:
                np.savetxt(csv, shap_values)
            with open(path.join(outputDir, f"shap_indices-threshold-{threshold}-{target}.csv"), "wb") as idx:
                np.savetxt(idx, x_include_labels, fmt="%d", delimiter=";")
    else:
        explainer = shap.explainers.Permutation(model.predict, x_train, max_evals="auto")
        shap_values = explainer(x_train)


        if save_values:
            with open(path.join(outputDir, f"shap_values-full-{target}.pickle"), "wb") as pkl:
                pickle.dump(shap_values, pkl)
